 ...
 * window down (tighter type and spacing) until it fits its share of the safe
 * band. Leaves the stage in `mode`.
 */
export function measureHud(hud: Hud, W: number, H: number, mode: 'full' | 'compact'): HudLayout {
  ...
  // measure with the window unclipped
  const clip = hud.win.style.clipPath
  hud.win.style.clipPath = 'none'
  const safe = rectOf(hud.probe)
  const band = Math.max(1, safe.y1 - safe.y0)
  // portrait: leave the upper part of the screen to the game
  const limit = portrait ? band * (H < 700 ? 0.8 : 0.74) : band
  stage.classList.remove('ct-compact')
  for (let i = 0; i < FIT.length && hud.win.offsetHeight > limit; i++) stage.classList.add(FIT[i])
  const full = rectOf(hud.win)
  stage.classList.add('ct-compact')
  const compact = rectOf(hud.win)
  stage.classList.toggle('ct-compact', mode === 'compact')
  hud.win.style.clipPath = clip
  return { portrait, safe, full, compact }
}
