...
const COPY: Record<string, () => string> = {
  ...
  contact: () => `
    ...
    <p>Elsewhere: ${ext(BRAND.classicSite, 'the classic 2026 site')} · ${ext(BRAND.orbitSite, 'the Orbit concept')} · ${ext(BRAND.resonanceSite, 'the Resonance concept')} · ${ext(BRAND.pressSite, 'the Press concept')} · ${ext(BRAND.townSite, 'the Town concept')}</p>
    <p>Credits: © ${new Date().getFullYear()} ${esc(BRAND.name)} · ${esc(BRAND.locale)}</p>
