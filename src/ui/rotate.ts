...
/*
 * Phone-landscape suggestion. Hark Arcade is framed in portrait on phones,
 * so a short, touch-first landscape viewport gets a game window instead of a
 * cramped screen: a pixel phone that turns upright in 90 degree steps,
 * "Turn your phone upright", and a "Continue anyway" button. Tablets and
 * laptops in landscape are taller than 500px and never see it.
 *
 ...
 * screen readers (only the chrome, stages and loader behind it are inert).
 *
...
const DISMISS_KEY = 'hark-arcade:rotate-ok'
const wasDismissed = () => {
  try {
...
export function mountRotateGate(onChange?: (shown: boolean) => void) {
  ...
  el.tabIndex = -1
  el.innerHTML = `
    <p class="rot-brand" aria-hidden="true"><span class="rot-brand-mark">${pixelMark(20)}</span><span class="rot-brand-text"><span class="rot-word">${WORDMARK}</span><span class="rot-tag">${CONCEPT_TAG}</span></span></p>
    <div class="rot-card">
      <div class="rot-art" aria-hidden="true">
        <span class="rot-arrow">${ICON.cursor()}</span>
        <span class="rot-phone">${ICON.phone()}</span>
      </div>
      <div class="rot-text">
        <p class="hud-eyebrow rot-eyebrow" aria-hidden="true">${MICROCOPY.signalEyebrow}</p>
        <h2 class="rot-title" id="rot-title">Turn your phone <em>upright.</em></h2>
        <p class="rot-sub" id="rot-sub">The arcade is built for portrait.</p>
        <p class="rot-actions"><button class="hud-btn rot-go" type="button">Continue anyway <span class="rot-go-ic" aria-hidden="true">${ICON.cursor()}</span></button></p>
      </div>
    </div>
  ...
  const sync = () => {
    ...
    if (on) {
      ...
      requestAnimationFrame(() => {
        if (on) live.textContent = 'Turn your phone upright. The arcade is built for portrait.'
      })
    } else {
      releaseInert('rotate')
      releaseScene('rotate')
...
/** The plain HTML booklet reads fine in any orientation. */
